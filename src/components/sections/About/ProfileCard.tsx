...
import {
  ...
  SiAmazon, SiTensorflow, SiExpress, SiPostgresql, SiMongodb
} from 'react-icons/si';
...
const expertiseRows: ExpertiseItem[][] = [
  [
  ...
  [
    { name: 'AI Integration', icon: SiTensorflow, color: '#FF6F00' },
    { name: 'REST APIs', icon: Layers, color: '#60A5FA' },
...
const socialLinks: SocialLinkItem[] = [
  { name: 'Email', icon: Mail, href: `mailto:${PERSONAL_INFO.email}`, label: 'Email Aditya', isExternal: false },
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="relative w-24 h-24 xs:w-28 xs:h-28 sm:w-36 sm:h-36 md:w-40 md:h-40 lg:w-48 lg:h-48 mb-3 xs:mb-4 sm:mb-5 md:mb-6">
          ...
          <div className="absolute inset-[2px] xs:inset-[3px] rounded-full overflow-hidden">
            <img
              src={`${PERSONAL_INFO.image}?t=${Date.now()}`}
              alt={profileAlt}
      ...
      <div>
        ...
        <div className="space-y-1.5 xs:space-y-2 sm:space-y-2.5">
          {expertiseRows.map((row, rowIndex) => (
            <div key={rowIndex} className="flex justify-center gap-1 xs:gap-1.5 sm:gap-2">
              {row.map((skill) => {
